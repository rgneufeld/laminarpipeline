-- Qualification approvals are first-class collaboration records. Internal
-- qualification rationale is never copied into client-facing approval notes.

alter table public.project_qualification_items
  add column if not exists client_approval_priority boolean not null default false;

alter table public.client_response_requests
  add column if not exists qualification_item_id uuid references public.project_qualification_items(id) on delete set null;

create table public.qualification_approval_groups (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  request_id uuid not null unique references public.client_response_requests(id) on delete cascade,
  created_by uuid not null references auth.users(id),
  created_at timestamptz not null default now()
);
create table public.qualification_approval_group_items (
  group_id uuid not null references public.qualification_approval_groups(id) on delete cascade,
  qualification_item_id uuid not null references public.project_qualification_items(id) on delete cascade,
  position integer not null default 0,
  primary key(group_id, qualification_item_id)
);
create index qualification_approval_group_items_qualification_idx on public.qualification_approval_group_items(qualification_item_id);

alter table public.qualification_approval_groups enable row level security;
alter table public.qualification_approval_group_items enable row level security;
create policy qualification_approval_group_read on public.qualification_approval_groups for select to authenticated using (public.is_internal_project_user(project_id) or public.is_client_project_user(project_id));
create policy qualification_approval_group_item_read on public.qualification_approval_group_items for select to authenticated using (exists(select 1 from public.qualification_approval_groups g where g.id = group_id and (public.is_internal_project_user(g.project_id) or public.is_client_project_user(g.project_id))));

create or replace function public.set_qualification_approval_priority(p_item uuid, p_priority boolean)
...
begin
  ...
  update public.project_qualification_items set client_approval_priority = coalesce(p_priority, false) where id = p_item;
  insert into public.audit_events(organisation_id, project_id, actor_id, event_type, entity_type, entity_id, payload)
  select organisation_id, v_item.project_id, auth.uid(), 'qualification.approval_priority_updated', 'project_qualification_item', p_item, jsonb_build_object('priority', coalesce(p_priority, false)) from public.projects where id = v_item.project_id;
end $$;
...
language plpgsql security definer set search_path = public as $$
declare v_project public.projects%rowtype; v_request public.client_response_requests%rowtype; v_group public.qualification_approval_groups%rowtype; v_item_id uuid; v_position integer := 0;
begin
  ...
  foreach v_item_id in array p_item_ids loop
    insert into public.qualification_approval_group_items(group_id, qualification_item_id, position) values(v_group.id, v_item_id, v_position);
...
begin
  ...
  if p_approval then
    ...
    if exists(
      ...
      join public.project_qualification_items q on q.id = gi.qualification_item_id
      where g.request_id = p_request and q.client_approval_priority
        and not exists(select 1 from public.client_response_requests r where r.qualification_item_id = q.id and r.status = 'approved')
    ) then raise exception 'all priority qualification approvals must be completed before this group can be approved'; end if;
  end if;
  ...
  select m.user_id, v_request.project_id, p_request, case when p_approval then 'client_approval_completed' else 'client_response_completed' end, case when p_approval then 'Client approval received: ' else 'Client response received: ' end || v_request.title, ''
...
revoke all on function public.set_qualification_approval_priority(uuid, boolean), public.request_qualification_approval(uuid, text, text, date, boolean, uuid[]), public.request_qualification_approval_group(uuid, text, text, uuid[], date, boolean, uuid[]), public.complete_client_response_request(uuid, boolean) from public;
grant execute on function public.set_qualification_approval_priority(uuid, boolean), public.request_qualification_approval(uuid, text, text, date, boolean, uuid[]), public.request_qualification_approval_group(uuid, text, text, uuid[], date, boolean, uuid[]) to authenticated;
grant execute on function public.complete_client_response_request(uuid, boolean) to authenticated;
grant select on public.qualification_approval_groups, public.qualification_approval_group_items to authenticated;
