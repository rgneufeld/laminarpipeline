...
function qualificationRecordHtml(definitionItem, row, linked, availableDocuments, clientDocuments) {
  ...
  return `<article class="qualification-record${row.complete ? ' complete' : ''}">
    ...
    <div class="qualification-client-instructions"><strong>Client instructions</strong><p>Visible to the client when this item is requested for approval.</p><textarea rows="2" maxlength="6000" data-qualification-client-instruction="${esc(row.id)}" placeholder="What does the client need to review, provide, or approve?">${esc(approvalRequest?.instructions || row.client_instruction || '')}</textarea><button class="btn btn-ghost btn-sm" type="button" data-action="qualification-client-instruction-save" data-item-id="${esc(row.id)}">Save client instructions</button></div>
