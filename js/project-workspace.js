...
sectionViews.details.addEventListener('submit', event => {
  ...
  if (form.dataset.action === 'client-response-request') {
    ...
    void persistSection(() => createClientResponseRequest({ projectId: workspace.project.id, taskId: String(data.get('taskId') || ''), title: String(data.get('title') || '').trim(), instructions: String(data.get('instructions') || '').trim(), requestType, dueOn: String(data.get('dueOn') || ''), requiresArtifact: data.get('requiresArtifact') === 'on', requiresSignedArtifact: data.get('requiresSignedArtifact') === 'on', approvalSubject, approvalVersion: String(data.get('approvalVersion') || '').trim() }));
    return;
  }
