...
import { addCycleTimeEntry, addCycleWorkItem, assignProjectMember, attachLinkedQualificationDocumentsToRequest, attachQualificationArtifact, cancelClientResponseRequest, closeOperatingCycle, completeClientResponseRequest, detachQualificationArtifact, loadProjectSections, loadProjectWorkspace, openOperatingCycle, removeArtifactUserAccess, removeProjectMember, requestQualificationApproval, requestQualificationApprovalGroup, saveQualificationClientInstruction, saveTaskNote, setArtifactUserAccess, setQualificationApprovalPriority, transitionTask, updateAssetItem, updateDeliverable, updateQualification, updateTraining } from './project-repository.js?v=0.1.2';
...
let artifactUploadFeedback = '';
let artifactUploadFeedbackKind = '';
let artifactUploadDraft = { title: '', visibility: 'internal' };
...
function auditDescription(event, qualificationDefs, qualificationRows) {
  ...
  if (event.event_type === 'client_request.opened') return { title: 'Client response requested', detail: event.payload?.title || 'Awaiting client response' };
  if (event.event_type === 'client_request.message_added') return { title: 'Client response note added', detail: 'Visible to the client and Laminar' };
  if (event.event_type === 'client_request.document_attached') return { title: 'Client response document attached', detail: 'Supporting document linked' };
  if (event.event_type === 'client_request.response_completed') return { title: 'Client completed a response', detail: event.payload?.title || 'Ready for Laminar follow-up' };
  if (event.event_type === 'client_request.approved') return { title: 'Client approval recorded', detail: event.payload?.approval_subject || event.payload?.title || 'Formal approval' };
  if (event.event_type === 'client_request.closed') return { title: 'Client response request closed', detail: event.payload?.title || '' };
...
function qualificationRecordHtml(definitionItem, row, linked, availableDocuments, clientDocuments) {
  ...
  return `<article class="qualification-record${row.complete ? ' complete' : ''}">
    ...
    <div class="qualification-approval-bar"><span class="qualification-approval-status${approvalStatus ? ` is-${esc(approvalStatus)}` : ''}">${esc(approvalLabel)}</span><label class="qualification-priority-toggle"><input type="checkbox" data-action="qualification-approval-priority" data-item-id="${esc(row.id)}" ${row.client_approval_priority ? 'checked' : ''}> Priority approval required</label>${['requested', 'responded'].includes(approvalStatus) ? `<button class="btn btn-ghost btn-sm" type="button" data-action="qualification-approval-attach-documents" data-request-id="${esc(approvalRequest.id)}">Include linked client documents</button>` : ''}${approvalStatus === 'responded' ? `<button class="btn btn-primary btn-sm" type="button" data-action="qualification-approval-complete" data-request-id="${esc(approvalRequest.id)}">Complete request</button>` : ''}${['requested', 'responded'].includes(approvalStatus) ? `<button class="btn btn-ghost btn-sm" type="button" data-action="qualification-approval-cancel" data-request-id="${esc(approvalRequest.id)}">Cancel request</button>` : ''}${!['requested', 'responded'].includes(approvalStatus) ? `<button class="btn btn-ghost btn-sm" type="button" data-action="qualification-approval-toggle" data-item-id="${esc(row.id)}">Request client approval</button>` : ''}</div>
    ${approvalOpen ? `<form class="qualification-approval-form" data-action="qualification-approval-request" data-item-id="${esc(row.id)}" data-title="${esc(definitionItem.label)}"><textarea name="clientNote" rows="2" maxlength="6000" placeholder="Client-facing approval note. Internal rationale below remains private.">${esc(row.client_instruction || '')}</textarea><label><input name="requiresSigned" type="checkbox"> Require a signed document upload before the client can respond</label><button class="btn btn-primary btn-sm" type="submit">Send approval request</button></form>` : ''}
    <details class="qualification-client-instructions"><summary><span>Client instructions</span><small>${esc(approvalRequest?.instructions || row.client_instruction || 'Not set')}</small></summary><div class="qualification-instruction-editor"><p>Visible to the client with this approval request.</p><textarea rows="2" maxlength="6000" data-qualification-client-instruction="${esc(row.id)}" placeholder="What does the client need to review, provide, or approve?">${esc(approvalRequest?.instructions || row.client_instruction || '')}</textarea><button class="btn btn-ghost btn-sm" type="button" data-action="qualification-client-instruction-save" data-item-id="${esc(row.id)}">Save client instructions</button></div></details>
    <div class="qualification-internal-note"><strong>Internal note</strong><textarea class="qualification-note" rows="2" data-qualification-note="${esc(row.id)}" placeholder="Internal rationale, decision context, or follow-up…">${esc(row.internal_note || '')}</textarea></div>
    <div class="qualification-evidence"><strong>Supporting documents</strong>${linked.length ? `<ul>${linked.filter(Boolean).map(doc => `<li><span>${esc(doc.title || 'Project document')}</span>${doc.visibility === 'client' ? '<em>Available to the client</em>' : (['requested', 'responded'].includes(approvalStatus) ? `<button type="button" class="text-button" data-action="qualification-document-share-client" data-item-id="${esc(row.id)}" data-artifact-id="${esc(doc.id)}" data-request-id="${esc(approvalRequest?.id || currentApprovalGroup()?.request?.id || '')}">Make available to client</button>` : '<em>Internal only</em>')} <button type="button" class="text-button" data-action="qualification-document-detach" data-item-id="${esc(row.id)}" data-artifact-id="${esc(doc.id)}">Remove</button></li>`).join('')}</ul>` : '<p>No supporting documents attached.</p>'}<p class="qualification-document-help">Making a document available creates a separate client-safe copy, keeps this original private, and attaches the copy to the active approval request.</p><div class="qualification-document-actions"><select data-qualification-document="${esc(row.id)}"><option value="">${availableDocuments.length ? 'Attach a project document…' : 'No uploaded project documents yet'}</option>${availableDocuments.filter(doc => !linked.some(attached => attached?.id === doc.id)).map(doc => `<option value="${esc(doc.id)}">${esc(doc.title)} · ${esc(doc.visibility)}</option>`).join('')}</select><button class="btn btn-ghost btn-sm" type="button" data-action="qualification-document-attach" data-item-id="${esc(row.id)}" ${availableDocuments.length ? '' : 'disabled'}>Attach document</button><button class="btn btn-ghost btn-sm" type="button" data-action="qualification-note-save" data-item-id="${esc(row.id)}">Save note</button></div></div>
  </article>`;
}
...
function renderSectionData(project, tasks) {
  ...
  sectionViews.artifacts.innerHTML = `<section class="project-data-panel artifact-upload-panel"><div><div class="section-label">Project documents</div><p class="project-data-note">Files are private by default, versioned rather than overwritten, and recorded in the project audit history.</p></div><form class="artifact-upload-form" data-action="artifact-upload"><input name="title" maxlength="180" placeholder="Document title (optional)" value="${esc(artifactUploadDraft.title)}"><select name="visibility"><option value="internal" ${artifactUploadDraft.visibility === 'internal' ? 'selected' : ''}>Internal only</option><option value="client" ${artifactUploadDraft.visibility === 'client' ? 'selected' : ''}>Client-visible after approval</option><option value="restricted" ${artifactUploadDraft.visibility === 'restricted' ? 'selected' : ''}>Restricted internal access</option><option value="client_upload" ${artifactUploadDraft.visibility === 'client_upload' ? 'selected' : ''}>Client-provided document</option></select><input name="file" type="file" required><button class="btn btn-primary btn-sm" type="submit">Upload document</button></form>${artifactUploadFeedback ? `<p class="artifact-upload-feedback ${esc(artifactUploadFeedbackKind)}" role="status">${esc(artifactUploadFeedback)}</p>` : ''}</section>${sections.artifacts.length ? `<div class="artifact-list">${sections.artifacts.map(artifact => { const versions = (artifact.artifact_versions || []).filter(version => !version.superseded_at).sort((a, b) => Number(b.version_number) - Number(a.version_number)); const current = versions[0]; const canPublish = ['internal', 'restricted'].includes(artifact.visibility) && artifact.status === 'available'; return `<article class="artifact-card"><div><div class="artifact-title">${esc(artifact.title)}</div><p class="project-data-note">${esc(artifact.visibility)} · ${esc(artifact.origin || 'upload')} · ${esc(artifact.status)}</p>${current ? `<p class="project-data-note">${esc(current.file_name)} · v${esc(current.version_number)}${current.byte_size ? ` · ${(Number(current.byte_size) / 1024).toFixed(1)} KB` : ''}</p>` : '<p class="project-data-note">Upload pending.</p>'}</div><div class="artifact-actions"><button class="btn btn-ghost btn-sm" type="button" data-action="artifact-download" data-artifact-id="${esc(artifact.id)}" ${current ? '' : 'disabled'}>Download</button>${canPublish ? `<button class="btn btn-ghost btn-sm" type="button" data-action="artifact-publish-client" data-artifact-id="${esc(artifact.id)}">Publish client copy</button>` : ''}<form class="artifact-version-form" data-action="artifact-version" data-artifact-id="${esc(artifact.id)}"><input name="file" type="file" required><button class="btn btn-ghost btn-sm" type="submit">Add version</button></form></div></article>`; }).join('')}</div>` : emptyState('No project documents have been uploaded yet.')}`;
  const restrictedArtifacts = sections.canManage ? sections.artifacts.filter(artifact => artifact.visibility === 'restricted') : [];
  if (restrictedArtifacts.length) sectionViews.artifacts.innerHTML += `<section class="project-data-panel artifact-grants-panel"><div class="section-label">Restricted document access</div><p class="project-data-note">Grant named internal users access to restricted documents. These grants are enforced in Supabase.</p>${restrictedArtifacts.map(artifact => { const grants = sections.artifactGrants.filter(grant => grant.artifact_id === artifact.id && grant.user_id); const granted = new Set(grants.map(grant => grant.user_id)); const available = sections.organisationMembers.filter(member => !granted.has(member.user_id)); return `<article class="artifact-grant-row"><strong>${esc(artifact.title)}</strong><form class="artifact-grant-form" data-action="artifact-grant" data-artifact-id="${esc(artifact.id)}"><select name="userId"><option value="">Grant a member…</option>${available.map(member => `<option value="${esc(member.user_id)}">${esc(memberName(member.user_id))} · ${esc(memberRoleLabel(member.role))}</option>`).join('')}</select><label><input name="canUpload" type="checkbox"> Add versions</label><label><input name="canApprove" type="checkbox"> Approve</label><button class="btn btn-ghost btn-sm" type="submit" ${available.length ? '' : 'disabled'}>Grant</button></form><div class="artifact-grant-list">${grants.length ? grants.map(grant => `<span>${esc(memberName(grant.user_id))} <button class="text-button" type="button" data-action="artifact-grant-remove" data-artifact-id="${esc(artifact.id)}" data-user-id="${esc(grant.user_id)}">Remove</button></span>`).join('') : 'No named access grants.'}</div></article>`; }).join('')}</section>`;
...
sectionViews.overview.addEventListener('click', event => {
  ...
  if ((button.dataset.action === 'qualification-group-approval-attach-documents' || button.dataset.action === 'qualification-approval-attach-documents') && button.dataset.requestId) {
    void persistSection(() => attachLinkedQualificationDocumentsToRequest({ requestId: button.dataset.requestId }));
    return;
  }
  ...
  if (button.dataset.action === 'qualification-client-instruction-save') {
    const instruction = record?.querySelector(`[data-qualification-client-instruction="${itemId}"]`)?.value || '';
    void persistSection(() => saveQualificationClientInstruction({ itemId, instruction }));
  }
  ...
  if (button.dataset.action === 'qualification-document-share-client' && button.dataset.artifactId && button.dataset.requestId) {
    if (!confirm('Create a client-safe copy and attach it to this approval request? The original document remains private.')) return;
    const artifactId = button.dataset.artifactId;
    const requestId = button.dataset.requestId;
    button.disabled = true;
    setStatus('Creating client-safe document copy…');
    void publishClientArtifactCopy({ projectId: workspace.project.id, artifactId }).then(async result => {
      if (!result?.artifactId) throw new Error('Client document copy was not created.');
      await attachQualificationArtifact({ itemId, artifactId: result.artifactId });
      await attachLinkedQualificationDocumentsToRequest({ requestId });
      await loadProject();
      setStatus('Client-safe document copy created and attached to the approval request.');
    }).catch(error => setStatus(error instanceof Error ? error.message : 'Unable to make the document available to the client.')).finally(() => { button.disabled = false; });
  }
...
sectionViews.artifacts.addEventListener('submit', event => {
  ...
  event.preventDefault();
  const data = new FormData(form);
  if (form.dataset.action === 'artifact-grant') {
    const userId = data.get('userId');
    if (!userId) { setStatus('Choose an organisation member first.'); return; }
    void persistSection(() => setArtifactUserAccess({ artifactId: form.dataset.artifactId, userId: String(userId), canView: true, canUploadVersion: data.get('canUpload') === 'on', canComment: false, canApprove: data.get('canApprove') === 'on', canManage: false }));
    return;
  }
  ...
  artifactUploadDraft = { title: String(data.get('title') || ''), visibility: String(data.get('visibility') || 'internal') };
  if (button) { button.disabled = true; button.textContent = 'Uploading…'; }
  artifactUploadFeedback = 'Uploading securely to Laminar Pipeline…';
  artifactUploadFeedbackKind = 'is-pending';
  render(workspace.project, workspace.tasks);
  ...
  void uploadProjectArtifact({
    ...
    artifactUploadDraft = { title: '', visibility: 'internal' };
    artifactUploadFeedback = `Document “${file.name}” uploaded. It is now available to attach to a qualification item.`;
    artifactUploadFeedbackKind = 'is-success';
    await loadProject();
    setStatus('Document uploaded to Supabase and recorded in the audit history.');
  }).catch(error => {
    artifactUploadFeedback = error instanceof Error ? `Upload failed: ${error.message}` : 'Upload failed. Please try again.';
    artifactUploadFeedbackKind = 'is-error';
    render(workspace.project, workspace.tasks);
    setStatus(artifactUploadFeedback);
  }).finally(() => {
    if (button) { button.disabled = false; button.textContent = isVersion ? 'Add version' : 'Upload document'; }
  });
});
sectionViews.artifacts.addEventListener('input', event => {
  const input = event.target.closest('input[name="title"]');
  if (input) artifactUploadDraft.title = input.value;
});
sectionViews.artifacts.addEventListener('change', event => {
  const select = event.target.closest('select[name="visibility"]');
  if (select) artifactUploadDraft.visibility = select.value;
});
sectionViews.artifacts.addEventListener('click', event => {
  const button = event.target.closest('[data-action]');
  if (!button?.dataset.artifactId || !workspace) return;
  const artifactId = button.dataset.artifactId;
  if (button.dataset.action === 'artifact-grant-remove' && button.dataset.userId) {
    void persistSection(() => removeArtifactUserAccess({ artifactId, userId: button.dataset.userId }));
    return;
  }
